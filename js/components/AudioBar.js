...
export function renderAudioBar(state) {
  ...
  return `
    <div class="fixed bottom-2 left-2 right-2 md:bottom-4 md:right-4 md:left-auto md:w-[420px] z-50 transition-all duration-300">
      <div class="bg-stone-950/95 dark:bg-stone-900/95 text-white backdrop-blur-xl rounded-2xl shadow-2xl border border-stone-800 p-2.5 sm:p-3 flex flex-col gap-2">
        
        <!-- Scrubber Progress Bar -->
        <div id="audio-scrubber" class="w-full bg-stone-800 h-1.5 rounded-full overflow-hidden cursor-pointer group">
          <div class="bg-amber-400 h-full rounded-full transition-all duration-75 relative group-hover:bg-amber-300" style="width: 0%;"></div>
        </div>
        ...
        <div class="flex items-center justify-between gap-2">
          ...
          <div class="flex items-center gap-2 overflow-hidden flex-1">
            <div class="w-8 h-8 rounded-xl bg-amber-500/20 text-amber-400 flex items-center justify-center flex-shrink-0 font-bold text-xs">
            ...
            <div class="flex flex-col min-w-0">
              <span class="text-xs font-bold text-stone-100 truncate">${verseText}</span>
              <span class="text-[10px] text-stone-400 truncate">${currentReciter.name}</span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-1 sm:gap-1.5 flex-shrink-0">
            <!-- Prev Verse -->
            <button id="btn-audio-prev" class="p-1.5 text-stone-300 hover:text-white hover:bg-stone-800 rounded-lg transition-colors" title="Ayat Sebelumnya">
              <svg viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg>
            </button>

            <!-- Play/Pause -->
            <button id="btn-audio-toggle" class="p-2 rounded-xl bg-amber-400 text-stone-950 hover:bg-amber-300 hover:scale-105 transition-all shadow-md" title="${isAudioPlaying ? 'Jeda' : 'Putar'}">
              ${isAudioPlaying ? `
            ...
            <!-- Next Verse -->
            <button id="btn-audio-next" class="p-1.5 text-stone-300 hover:text-white hover:bg-stone-800 rounded-lg transition-colors" title="Ayat Berikutnya">
              <svg viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg>
            </button>

            <!-- Speed Button -->
            <button id="btn-audio-speed" class="px-1.5 py-1 text-[11px] font-bold text-amber-300 hover:bg-stone-800 rounded-lg transition-colors" title="Kecepatan Audio">
              ${playbackRate}x
            </button>
